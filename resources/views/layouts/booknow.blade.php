                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-lg-6 col-xl-6">
                                ...
                                <p class="text-truncate mb-0">{{ Str($hotel->hotel_description, 200) }}</p>
                            </div>
                            ...
                            <div class="col-md-6 col-lg-3 col-xl-3 border-sm-start-none border-start d-flex flex-column justify-content-center gap-2">
                                ...
                                <button class="btn btn-outline-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#galleryModal{{ $hotel->id }}">
                                    View Photo
                                </button>
                                <button class="btn btn-info btn-sm text-white" data-bs-toggle="modal" data-bs-target="#infoModal{{ $hotel->id }}">
                                    More Info
                                </button>

                                @if($hotel->price)
                                <div class="mt-3">
                                    <h6 class="text-success">From ${{ $hotel->price }}</h6>
                                </div>
                                @endif
                            </div>
...
@auth
@foreach($hotels as $hotel)
<div class="modal fade" id="bookingModal{{ $hotel->id }}" tabindex="-1" aria-labelledby="bookingModalLabel{{ $hotel->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content rounded shadow">
            <div class="modal-header">
                <h5 class="modal-title" id="bookingModalLabel{{ $hotel->id }}">
                    Book a Room at {{ $hotel->hotel_name }}
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <div class="modal-body">
                <form action="{{ route('bookings.store') }}" method="POST" id="bookingForm{{ $hotel->id }}">
                    @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" id="total{{ $hotel->id }}" class="form-control total-price fw-bold text-success" readonly placeholder="Total Price">
                        </div>
                    </div>
...
@foreach($hotels as $hotel)
<div class="modal fade" id="galleryModal{{ $hotel->id }}" tabindex="-1" aria-labelledby="galleryModalLabel{{ $hotel->id }}" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content rounded shadow">
            <div class="modal-header border-0">
                <h5 class="modal-title" id="galleryModalLabel{{ $hotel->id }}">
...
@endforeach
@foreach($hotels as $hotel)
<div class="modal fade" id="infoModal{{ $hotel->id }}" tabindex="-1" aria-labelledby="infoModalLabel{{ $hotel->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content rounded shadow">
            <div class="modal-header">
                <h5 class="modal-title" id="infoModalLabel{{ $hotel->id }}">
            ...
            <div class="modal-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <h5 class="fw-bold mb-2">{{ $hotel->hotel_name }}</h5>

                        
                        <div class="mb-2">
                            @for ($i = 0; $i < floor($hotel->hotel_rating ?? 0); $i++)
                                <i class="fa fa-star text-warning"></i>
                            @endfor
                            <span class="ms-2 text-muted">({{ $hotel->hotel_rating ?? 'N/A' }})</span>
                        </div>
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#bookingModal{{ $hotel->id }}">Book Now</button>
            </div>
        </div>
    </div>
</div>
